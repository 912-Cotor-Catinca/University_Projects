...
void GUI::initGUI()
{
	...
	this->watchlistModel = new MyModel{ suser };
	...
	this->ctrl_r = new QShortcut(this);
	this->ctrl_z = new QShortcut(this);
	
	this->ctrl_r->setKey(Qt::CTRL + Qt::Key_R);
	this->ctrl_z->setKey(Qt::CTRL + Qt::Key_Z);
	setButtons();
	...
	buttonsArea->addWidget(this->undoBTN, 0, 3);
	buttonsArea->addWidget(this->redoBTN, 0, 4);
...
void GUI::setButtons()
{
	...
	this->undoBTN->setStyleSheet("QPushButton{font-size: 18px;font-family: Times New Roman;color: rgb(255, 255, 255);background-color: rgb(38,56,76);}");
	this->redoBTN->setStyleSheet("QPushButton{font-size: 18px;font-family: Times New Roman;color: rgb(255, 255, 255);background-color: rgb(38,56,76);}");
}
...
void GUI::SignalsAndSlots()
{
	...
	QObject::connect(this->undoBTN, &QPushButton::clicked, this, &GUI::Undo);
	QObject::connect(this->redoBTN, &QPushButton::clicked, this, &GUI::Redo);
	QObject::connect(this->ctrl_r, &QShortcut::activated, this, &GUI::Redo);
	QObject::connect(this->ctrl_z, &QShortcut::activated, this, &GUI::Undo);
}
...
void GUI::addTutorial()
{
	...
	try {
		...
		MasterC elem = MasterC(title, presenter, duration, link, like);
		this->cmd->ExecuteCommand(new AddComman(this->serv.GetRepo(), elem));
...
void GUI::deleteTutorial()
{
	...
	this->cmd->ExecuteCommand(new DeleteCommand(this->serv.GetRepo(), c));
...
void GUI::updateTutorial()
{
	...
	try {
		...
		string duration = this->durationLE->text().toStdString();
		string likes = this->likesLE->text().toStdString();
		this->cmd->ExecuteCommand(new UpdateCommand(this->serv.GetRepo(), c, title, presenter, duration, c.getLink(), stoi(likes)));
...
void GUI::CSVlistdisplay()
{
	...
	this->watchlist->setModel(this->watchlistModel);
