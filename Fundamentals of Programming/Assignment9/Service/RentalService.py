...
class RentalService:
    ...
    def find_client_id(self, id_client):
        for i in range(len(self._rental_repo)):
            if self._rental_repo[i].client == id_client:
                return i
        return -1
    ...
    def most_rented_books(self):
        ...
        result.sort(reverse=True, key=lambda book_dto: book_dto.count)
        return result
    ...
    def most_clients(self):
        ...
        for client in self._client_repo.get_all():
            ...
            for rental in self._rental_repo.get_all():
                if client.client_id == rental.client:
                    count += len(rental)
    ...
    def _create_book_rental_count(self, book):
        return len(self.filter_rentals(None, book))

    def _create_client_count(self, client):
        return len(self.filter_rentals(client, None))
