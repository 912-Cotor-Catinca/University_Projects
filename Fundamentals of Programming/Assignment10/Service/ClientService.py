...
class ClientService:
    ...
    def search_client_name(self, name):
        return Sort.filter(self._client_repo, lambda client: name.lower() in client.name.lower())

    def search_id(self, client_id):
        id = str(client_id)
        return Sort.filter(self._client_repo, lambda client: id.lower() in str(client.client_id).lower())
